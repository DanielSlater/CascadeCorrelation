...
class _CandidateNode(object):
    def __init__(self, connection_weights: numpy.array):
...
class CascadeNet(object):
    ...
    def _get_output_node_activation(self, connections: numpy.array):
        return self._get_output_node_activation_from_activations(connections, self._activations[:len(connections)])

    def _get_output_node_activation_from_activations(self, connections: numpy.array, activations: numpy.array):
        return self.function(numpy.inner(connections, activations))

    def _feed_forward_hidden_nodes(self, input: [float]):
    ...
    def _feed_forward(self, inputs: [float]):
    ...
    def _generate_candidate(self, inputs: [float], targets: [float]):
        ...
        while epochs < self.train_candidates_max_epochs and iterations_without_improvement < 3:
            ...
            for candidate in candidates:
                ...
                derivatives = self._correlation_derrivetive(candidate.weights,
                                                            errors,
                                                            mean_errors,
    ...
    def train(self, inputs: [], targets: [],
              stop_error_threshold: float=-sys.float_info.max,
              max_hidden_nodes: int=10,
    ...
    def _back_propagation(self, result: [float], target: [float]):
        ...
        for input in range(self._non_output_nodes()):
            for output in range(len(result)):
                dwde = self._activations[input] * self.d_function(result[output]) * output_errors[output]
                derivatives[output][input] += dwde
        return derivatives

    @staticmethod
    def _real_correlations(candidate_activation: [float], errors: [[float]]) -> [float]:
    ...
    @staticmethod
    def _correlations(candidate_activation: [float], errors: [[float]], mean_errors: [float]) -> float:
    ...
    @staticmethod
    def _correlation(candidate_activation: [float], errors: [[float]], mean_errors: [float]) -> float:
        sum = 0.0
        for p in range(len(candidate_activation)):
            for o in range(len(errors[0])):
                x = candidate_activation[p] * (errors[p][o] - mean_errors[o])
    ...
    def _correlation_derrivetive(self, candidate_weights: numpy.array,
                                 errors: list,
                                 mean_errors: list,
                                 candidate_activations: list,
                                 sign_correlation_with_outputs: [float]):
        derrivatives = []
    ...
    def backprop_train_till_convergence(self, inputs, targets,
                                        mini_batch_size: int=10,
                                        stop_error_threshold: float=0.05,
                                        max_iterations_per_epoch: int=2000):
...
def print_results(network: CascadeNet, inputs: [[float]], targets: [[float]]):
